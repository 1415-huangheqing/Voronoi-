...
def debugDrawPath(triangles, p):
    ...
    for t in triangles:
        tn = []
        for i in range(3):
            tn.append(tuple(map(lambda y: y*50, list(t[i]))))
        color = getPolygonColor(t, im)
        draw.polygon((tuple(tn[0]), tuple(tn[1]), tuple(tn[2])), fill=color, outline='white')
...
def getPolygonColor(pol, im):
    black_count = 0  # 记录黑色像素的数量
    white_count = 0  # 记录白色像素的数量

    # 统计该 Voronoi 区域内的黑白像素数量
    for p in pol:
        if p[0] >= im.size[0] or p[0] < 0 or p[1] >= im.size[1] or p[1] < 0:
            continue
        pixel_color = im.getpixel(p)

        if pixel_color == 0:  # 黑色像素
            black_count += 1
        elif pixel_color == 255:  # 白色像素
            white_count += 1
